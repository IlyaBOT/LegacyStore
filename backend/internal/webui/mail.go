...
func sendRecoveryEmail(cfg config.Config, recipient, token string) error {
	...
	if baseURL == "" {
		baseURL = strings.TrimRight(cfg.PublicBaseURL, "/") + "/account"
	}
	separator := "?"
	if strings.Contains(baseURL, "?") {
		separator = "&"
	}
	link := baseURL + separator + "mode=reset&token=" + url.QueryEscape(token)
